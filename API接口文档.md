...
震源直线距离（考虑地球曲率，R = 6371.0088km）：

```
lineDis = √((R - depth)² + R² - 2(R - depth)·R·cos(dis / R))
```

断层尺度修正与等效震源距：

```
faultLength = 10 ^ ((M - 3.821) / 1.86)
hypoDis = max(lineDis - 10 - faultLength, dis - faultLength, 0.2 × (lineDis - 10), 0)
```

烈度估计（震中距与等效震源距两次计算的均值）：

```
CEA(M, d) = 1.297M - 4.368 × log10(d + 15) + 5.363
烈度 = (CEA(M, dis) + CEA(M, hypoDis)) / 2
```

深度缺失或小于 10km 时按 10km 计；本地烈度保留一位小数，震中预估烈度取 0~12 的整数等级。

#### 日本气象厅震度（适用于 JMA/CWA）

```
Mw = Mj - 0.171
faultLength = 10 ^ (0.5·Mw - 1.85) / 2
x = max(lineDis - faultLength, 3)
PGV600 = 10 ^ (0.58·Mw + 0.0038·depth - 1.29 - log10(x + 0.0028 × 10 ^ (0.5·Mw)) - 0.002x)
PGV = PGV600 × 1.307 × Arv
计测震度 = 2.68 + 1.72 × log10(PGV)
```

无场地增幅数据时 Arv 取 1（工程基岩近似）。官方已发布最大烈度/震度时优先使用官方值，
仅在官方字段缺失（如福建预警、CENC 速报 intensity 为空）时使用上述公式估算震中烈度。
